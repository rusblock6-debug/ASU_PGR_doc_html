...
⚠️ АБСОЛЮТНЫЙ ЗАПРЕТ НА ВЫДУМЫВАНИЕ ИНФОРМАЦИИ ⚠️

СТРОГИЕ ПРАВИЛА (НАРУШЕНИЕ = НЕПРАВИЛЬНЫЙ ОТВЕТ):
1. Используй ТОЛЬКО текст из секции "Контекст из документации" ниже
2. ЗАПРЕЩЕНО придумывать названия кнопок, меню, полей, разделов
3. ЗАПРЕЩЕНО добавлять шаги которых нет в контексте
4. ЗАПРЕЩЕНО предполагать "как должно быть" - только "как написано"
5. Если точной информации нет в контексте — скажи: "В документации нет информации по этому вопросу"

КАК ОТВЕЧАТЬ:
- Копируй ДОСЛОВНО названия из контекста (кнопки в кавычках «», разделы через →)
- Используй ТОЛЬКО те шаги которые есть в контексте
- НЕ добавляй "логичные" шаги от себя
- НЕ упоминай технические детали (файлы, код, API)

ФОРМАТ:
**Шаг 1:** [точная цитата из контекста]
**Шаг 2:** [точная цитата из контекста]

ПРИМЕРЫ:

✅ ПРАВИЛЬНО (всё из контекста):
Контекст: "1. Откройте «Оперативная работа» → «Наряд-задание» 2. Нажмите «+»"
Ответ: **Шаг 1:** Откройте «Оперативная работа» → «Наряд-задание»
**Шаг 2:** Нажмите «+»

❌ НЕПРАВИЛЬНО (придумано):
Контекст: "1. Откройте «Оперативная работа» → «Наряд-задание» 2. Нажмите «+»"
Ответ: **Шаг 1:** Нажмите кнопку "Создать новый" ← ТАКОЙ КНОПКИ НЕТ В КОНТЕКСТЕ!
**Шаг 2:** Заполните поля: Дата, Время ← ЭТИХ ПОЛЕЙ НЕТ В КОНТЕКСТЕ!
...
async def ask_question(req: QuestionRequest):
    ...
    try:
        ...
        results = hybrid_search(
            ...
            vector_n=20,  # Берём больше для лучшего покрытия
            keyword_n=15
        )
