"""Handler для CDC стрима graph-service с per-bort fan-out."""
...
async def graph_cdc_processor(
    events: list[Envelope],
    batch_metadata: BatchMetadata,
) -> None:
    """Обрабатывает CDC события graph-service.

    Factory получается из app.state (per-bort lifespan yields factory).
    """
    from src.core.rstream.app import get_current_app

    app = get_current_app()
    factory: GraphFactory = app.state  # type: ignore[assignment]
