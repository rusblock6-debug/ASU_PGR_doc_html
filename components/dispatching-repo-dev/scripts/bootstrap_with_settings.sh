...
start_settings_service() {
  ...
  log "settings-bort runtime user: ${runner_uid}:${runner_gid}"
  touch "${OUTPUT_ENV_PATH}"
  env UID="${runner_uid}" GID="${runner_gid}" \
    BORT_ENV_OUTPUT_PATH="${OUTPUT_ENV_CONTAINER_PATH}" \
    "${cmd[@]}" up -d --build "${SETTINGS_SERVICE}"
}
