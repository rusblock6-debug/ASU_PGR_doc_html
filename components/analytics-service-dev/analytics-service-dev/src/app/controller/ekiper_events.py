...
from src.app.repository import EkiperEventsRepository, S3FileRepository
from src.core.s3 import S3Client


class EkiperEventsController:
    def __init__(
        self,
        ekiper_events_repository: EkiperEventsRepository,
        s3_file_repository: S3FileRepository,
        s3_client: S3Client,
    ) -> None:
        self.ekiper_events_repository = ekiper_events_repository
        self.s3_file_repository = s3_file_repository
        self.s3_client = s3_client

    async def load(self, object_key: str, e_tag: str, filename: str) -> None:
        """Load file from ekiper events repository.
        ...
        await self.ekiper_events_repository.load_parquet(url)
