...
from src.app.broker_factory import BrokerFactory
from src.app.controller import CycleTagHistoryController, EkiperEventsController, GpsDataController
...
async def get_minio_file(
    ...
    ekiper_events_controller: Annotated[
        ...
        Depends(BrokerFactory.get_ekiper_events_controller),
    ],
    gps_data_controller: Annotated[
        GpsDataController,
        Depends(BrokerFactory.get_gps_data_controller),
    ],
    cycle_tag_history_controller: Annotated[
        CycleTagHistoryController,
        Depends(BrokerFactory.get_cycle_tag_history_controller),
    ],
) -> None:
    ...
    if is_ekiper_event(event):
        ...
        if "cycle_tag_history" in event.key:
            await cycle_tag_history_controller.load(
                event.key,
                default_record.s3.object.e_tag,
                default_record.s3.object.key,
            )
