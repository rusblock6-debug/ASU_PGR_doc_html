...
interface NewShiftTaskPopupProps {
  ...
  /** Подтверждение задания (приступить). */
  readonly onConfirm: () => void | Promise<void>;
  /** Отказ от задания. */
  readonly onReject: () => void | Promise<void>;
  /** Флаг выполнения мутации (блокирует кнопки). */
...
export const NewShiftTaskPopup = ({ routeTask, onConfirm, onReject, isLoading }: NewShiftTaskPopupProps) => {
  ...
  const weightText = hasValue(routeTask.weight) ? `${routeTask.weight} т` : NO_DATA.LONG_DASH;
  const volumeText = hasValue(routeTask.volume) ? `${routeTask.volume} м³` : NO_DATA.LONG_DASH;
