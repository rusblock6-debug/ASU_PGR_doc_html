...
import { selectVehicleState, useSetVehicleStateTransitionMutation } from '@/shared/api/endpoints/vehicle-state';
import DowntimeIcon from '@/shared/assets/icons/ic-downtime.svg?react';
...
export const VehicleStatusPage = () => {
  ...
  const normalizedSelectedIndex =
    listSelectedIndex >= 0 && listSelectedIndex < STANDARD_STATUS_CHAIN_LEN ? listSelectedIndex : 0;
  const nextForwardIndex =
    normalizedSelectedIndex >= STANDARD_STATUS_CHAIN_LEN - 1 ? null : normalizedSelectedIndex + 1;
  const nextForwardState = nextForwardIndex == null ? null : STANDARD_STATUS_CHAIN[nextForwardIndex].en;

  const canGoForward = nextForwardState != null && !isTransitioning;
  ...
  const handleForward = () => {
    if (!nextForwardState || nextForwardIndex == null) {
      return;
    }
    void setTransition({ new_state: nextForwardState, reason: 'manual', comment: '' })
      .unwrap()
      .then(() => {
        setListSelectedIndex(nextForwardIndex);
      });
  };
