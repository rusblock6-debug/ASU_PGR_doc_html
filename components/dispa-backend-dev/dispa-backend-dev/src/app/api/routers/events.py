...
async def combined_event_stream(vehicle_id: str, request: Request) -> AsyncGenerator[str]:
    ...
    Подписывается на 4 Redis-канала:
    ...
    await pubsub.subscribe(state_channel, alert_channel, location_channel, weight_channel)
    ...
    try:
        ...
        await pubsub.unsubscribe(state_channel, alert_channel, location_channel, weight_channel)
        await pubsub.close()
