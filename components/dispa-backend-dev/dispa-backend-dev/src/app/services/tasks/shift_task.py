...
class ShiftTaskService:
    ...
    async def preview_from_previous_shift(
        ...
        for prev_task in prev_shift_tasks:
            ...
            # Исключить задания, где есть RouteTask со статусом COMPLETED
            has_completed_routes = any(
                rt.status == TripStatusRouteEnum.COMPLETED for rt in (prev_task.route_tasks or [])
            )

            if has_completed_routes:
                logger.debug(
                    f"Skipping task {prev_task.id} - has completed routes",
                )
                continue
    ...
    async def copy_from_previous_shift(
        ...
        Логика:
        - На cold start только сохраняет текущее состояние смен по активным режимам работы
        - При последующих вызовах сравнивает номер смены для каждого режима работы
        - Если номер смены изменился, получает задания из предыдущей смены,
