...
async def load_tasks_to_redis_queue(
    vehicle_id: str,
    tasks: list[RouteTask],
) -> None:
    """Загрузить задания в Redis Sorted Sets.

    - ZADD trip-service:task_queue:ordered (по order)
    - ZADD trip-service:task_queue:{place_a_id} (по месту старта)

    Args:
        vehicle_id: ID транспорта
        tasks: Список заданий для загрузки
    """
    try:
        for task in tasks:
            if task.status == TripStatusRouteEnum.DELIVERED:
                await redis_client.add_task_to_queue(
                    vehicle_id=vehicle_id,
                    task_id=str(task.id),
                    order=task.route_order,
                    start_point_id=str(task.place_a_id),  # place_id в Redis key
                )

        logger.info(
            "RouteTasks loaded to Redis queue",
            vehicle_id=vehicle_id,
            count=len([t for t in tasks if t.status == TripStatusRouteEnum.DELIVERED]),
        )

    except Exception as e:
        logger.error(
            "Failed to load tasks to Redis",
            vehicle_id=vehicle_id,
            error=str(e),
            exc_info=True,
        )
