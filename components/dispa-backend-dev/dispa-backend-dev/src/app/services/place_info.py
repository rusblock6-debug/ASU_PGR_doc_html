...
async def get_place_stock(place_id: int, db: AsyncSession) -> float:
    """Получить актуальный остаток по месту, суммируя все change_amount из place_remaining_history.
    ...
    Returns:
        float: Сумма всех change_amount для данного place_id, или 0.0 если записей нет
    """
    try:
        query = select(func.sum(PlaceRemainingHistory.change_amount)).where(PlaceRemainingHistory.place_id == place_id)
