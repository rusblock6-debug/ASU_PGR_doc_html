...
5. Для каждого маршрута:
   ...
   - volume_fact  = определяется по рейсам (Trip) за смену:
                    ...
                    SUM(ABS(change_amount)) по unloading-записям
...
async def get_fleet_control(db: AsyncSession) -> FleetControlResponse:
    ...
    no_task_objects = [make_vehicle(vid, is_assigned=True) for vid in unused.no_task]
    ...
    garages_by_id: dict[int, list[FleetVehicle]] = {
        park_id: list(vehicles_by_id.values()) for park_id, vehicles_by_id in garages_objects.items()
    }
    ...
    idle_objects = [make_vehicle(vid, is_assigned=True) for vid in unused.idle]
...
async def _calc_volume_fact_by_trips(
    ...
    4. SUM(ABS(change_amount)) — фактический объём unloading по маршруту.
...
async def create_empty_route_template(
    ...
    Нельзя создать дубль маршрута с теми же ПП/ПР в рамках одной смены.
    """
    shift_info = await _get_current_shift_info()
    if not shift_info:
