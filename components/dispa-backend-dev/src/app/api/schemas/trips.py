...
class TripValidationMixin:
    ...
    @field_validator("change_volume", "change_weight")
    @classmethod
    def validate_change_not_negative(
        cls,
        value: float | None,
        info: ValidationInfo,
    ) -> float | None:
        """Запрещает отрицательные значения объема и веса рейса."""
...
class TripCreate(TripValidationMixin, BaseModel):
    ...
    change_volume: float | None = Field(
        None,
        description=("Фактический объем рейса (м3) для пересчета остатков."),
    )
    change_weight: float | None = Field(
        None,
        description=("Фактический вес рейса (тонны)."),
    )
    load_type_id: int | None = Field(
        None,
        description=(
            "ID вида груза (load_type.id). "
            "Если не указан, определяется автоматически по пункту погрузки."
        ),
    )
...
class TripUpdate(TripValidationMixin, BaseModel):
    ...
    vehicle_id: int = Field(..., description="ID транспорта")
    loading_place_id: int | None = Field(
        ...,
        description="ID места погрузки (place.id), может быть не определен",
    )
    loading_timestamp: datetime = Field(..., description="Время погрузки")
    unloading_place_id: int | None = Field(
        ...,
        description="ID места разгрузки (place.id), может быть не определен",
    )
    ...
    change_volume: float | None = Field(
        None,
        description=(
            "Фактический объем рейса (м3) для пересчета остатков. "
    ...
    load_type_id: int | None = Field(
        ...
        description=(
            "ID вида груза (load_type.id). "
            "Если задан, при пересчете change_volume/change_weight используется его плотность."
        ),
    )
...
class TripResponse(APIBaseModel):
    ...
    change_volume: float | None = Field(None, description="Объем рейса (м3)")
    change_weight: float | None = Field(None, description="Вес рейса (тонны), рассчитанный по типу руды")
    load_type_id: int | None = Field(
        None,
        description="ID вида груза (load_type.id), использованный для расчета объема/веса",
    )
