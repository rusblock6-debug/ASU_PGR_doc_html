...
class VehicleTooltipResponse(APIBaseModel):
    ...
    weight: float | None = Field(
        None,
        description=(
            "Нормативная грузоподъёмность модели техники (т), load_capacity_tons из enterprise-service; "
            "null если модель не задана или поле пустое"
        ),
    )
    ...
    last_message_timestamp: str | None = Field(
        None,
        description=(
            "Время последнего сообщения в стримах telemetry-service (gps, speed), "
