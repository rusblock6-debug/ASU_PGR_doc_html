...
class CurrentShiftStatsResponse(BaseModel):
    ...
    actual_volume_sum: int = Field(
        ...,
        description="Суммарный фактический объем за смену (положительные change_volume из place_remaining_history)",
    )
    planned_volume_sum: int = Field(
        ...,
        description="Суммарный плановый объем за смену (SUM route_tasks.volume)",
    )
    actual_weight_sum: int = Field(
        ...,
        description="Суммарный фактический вес за смену (положительные change_weight из place_remaining_history)",
    )
...
class LastStateResponse(BaseModel):
    """Cтатус рейса в определенный момент времени."""

    state: str = Field(..., description="Состояние")
    place_id: int | None = Field(..., description="ID места")
