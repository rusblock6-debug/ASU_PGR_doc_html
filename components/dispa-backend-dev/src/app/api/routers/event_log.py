...
async def get_current_shift_stats(
    ...
    planned_query = select(
        ...
        func.coalesce(func.sum(RouteTask.volume), 0.0),
    ...
    planned_trips_count_raw, planned_volume_raw, planned_weight_raw = planned_result.one()

    volume_conditions = [
        PlaceRemainingHistory.timestamp >= shift_start,
        PlaceRemainingHistory.timestamp < effective_end,
        PlaceRemainingHistory.change_volume > 0,
    ]
    if vehicle_id is not None:
        volume_conditions.append(
            func.coalesce(
    ...
    actual_volume_query = (
        select(func.coalesce(func.sum(PlaceRemainingHistory.change_volume), 0.0))
        .select_from(PlaceRemainingHistory)
        .outerjoin(Trip, Trip.cycle_id == PlaceRemainingHistory.cycle_id)
        .where(*volume_conditions)
    )
    actual_volume_raw = (await session.execute(actual_volume_query)).scalar_one()

    weight_conditions = [
        PlaceRemainingHistory.timestamp >= shift_start,
        PlaceRemainingHistory.timestamp < effective_end,
        PlaceRemainingHistory.change_weight > 0,
    ]
    if vehicle_id is not None:
    ...
    # Фактический вес считаем строго по timestamp-интервалу смены, без привязки к shift_id.
    actual_weight_query = (
        select(func.coalesce(func.sum(PlaceRemainingHistory.change_weight), 0.0))
        .select_from(PlaceRemainingHistory)
        .outerjoin(Trip, Trip.cycle_id == PlaceRemainingHistory.cycle_id)
    ...
    return CurrentShiftStatsResponse(
        ...
        actual_volume_sum=int(round(float(actual_volume_raw or 0.0))),
        planned_volume_sum=int(round(float(planned_volume_raw or 0.0))),
        actual_weight_sum=int(round(float(actual_weight_raw or 0.0))),
        planned_weight_sum=int(round(float(planned_weight_raw or 0.0))),
    )


@router.get("/state-history/last-state", response_model=LastStateResponse)
async def get_last_state(
    session: SessionDepends,
    vehicle_id: int = Query(),
    timestamp: datetime = Query(),
) -> CycleStateHistory:
    """Получить последнее состояние техники на указанный момент времени."""
    query = (
        select(CycleStateHistory)
        .where(
            CycleStateHistory.vehicle_id == vehicle_id,
            CycleStateHistory.timestamp <= timestamp,
        )
        .order_by(CycleStateHistory.timestamp.desc())
        .limit(1)
    )
    result = await session.execute(query)
    row = result.scalar_one_or_none()
    if row is None:
        raise HTTPException(status_code=404, detail="No state found for this vehicle at the given timestamp")
    return row
