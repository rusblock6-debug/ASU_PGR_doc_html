...
from app.services.trip_manager import (
    bulk_update_trips_cycle_num,
    get_place_remaining_history_by_trips,
)
...
async def _get_places_names(place_ids: list[int]) -> dict[int, str]:
    """Получить названия мест из graph-service."""
...
def _extract_trip_change_values(
    history_list: Sequence[PlaceRemainingHistory],
) -> tuple[float | None, float | None, int | None]:
    """Извлечь модуль объема и веса из истории остатков за один проход."""
    change_volume: float | None = None
    change_weight: float | None = None
    load_type_id: int | None = None

    for history in history_list:
        if change_volume is None and history.change_volume is not None:
            change_volume = abs(history.change_volume)
        if change_weight is None and history.change_weight is not None:
            change_weight = abs(history.change_weight)
        if load_type_id is None and history.load_type_id is not None:
            load_type_id = history.load_type_id
        if change_volume is not None and change_weight is not None and load_type_id is not None:
            break

    return change_volume, change_weight, load_type_id
...
async def list_trips(
    ...
    try:
        ...
        completed_trips = [trip for trip in trips if trip.end_time is not None]

        # Получаем названия мест и историю изменений остатков только для завершенных рейсов
        places_map = await _get_places_names(place_ids) if place_ids else {}
        place_remaining_history_map = (
            await get_place_remaining_history_by_trips(completed_trips, session) if completed_trips else {}
        )
        ...
        for trip in trips:
            ...
            # Вычисляем объем/вес по истории остатков, если рейс завершен
            # и есть хотя бы одна запись в place_remaining_history
            ...
            if is_trip_completed and len(history_list) >= 1:
                ...
                change_volume, change_weight, load_type_id = _extract_trip_change_values(history_list)
                if change_volume is not None:
                    trip_dict["change_volume"] = change_volume
                if change_weight is not None:
                    trip_dict["change_weight"] = change_weight
                if load_type_id is not None:
                    trip_dict["load_type_id"] = load_type_id
...
async def create_trip(
    ...
    try:
        ...
        if trip_data.change_volume is not None or trip_data.change_weight is not None:
            ...
            if trip_data.change_volume is not None:
                loading_kwargs["change_volume"] = -trip_data.change_volume
                unloading_kwargs["change_volume"] = trip_data.change_volume
            if trip_data.change_weight is not None:
                loading_kwargs["change_weight"] = -trip_data.change_weight
                unloading_kwargs["change_weight"] = trip_data.change_weight
            if trip_data.load_type_id is not None:
                loading_kwargs["load_type_id"] = trip_data.load_type_id
                unloading_kwargs["load_type_id"] = trip_data.load_type_id

            # Одна транзакция: при ошибке на второй записи не остаётся «висячей» только loading
            await place_remaining_service.create_trip_loading_unloading_pair(
                db=session,
                loading_data=PlaceRemainingHistoryCreate(**loading_kwargs),
                unloading_data=PlaceRemainingHistoryCreate(**unloading_kwargs),
                notify_graph=True,
            )
...
async def get_trip(
    ...
    try:
        ...
        places_map = await _get_places_names(place_ids) if place_ids else {}
        place_remaining_history_map = (
            await get_place_remaining_history_by_trips([trip], session) if trip and trip.end_time is not None else {}
        )
        ...
        response_data["cycle_num"] = trip.cycle_num
        response_data["trip_type"] = trip.trip_type
        response_data["start_time"] = trip.start_time
        response_data["end_time"] = trip.end_time
        response_data["loading_place_id"] = trip.loading_place_id
        response_data["unloading_place_id"] = trip.unloading_place_id
        response_data["loading_tag"] = trip.loading_tag
        response_data["unloading_tag"] = trip.unloading_tag
        response_data["loading_timestamp"] = trip.loading_timestamp
        response_data["unloading_timestamp"] = trip.unloading_timestamp
        response_data["loading_place_name"] = places_map.get(trip.loading_place_id) if trip.loading_place_id else None
        response_data["unloading_place_name"] = (
            places_map.get(trip.unloading_place_id) if trip.unloading_place_id else None
        )

        history_list = place_remaining_history_map.get(trip.cycle_id, []) if trip.end_time else []
        if history_list:
            change_volume, change_weight, load_type_id = _extract_trip_change_values(history_list)
            if change_volume is not None:
                response_data["change_volume"] = change_volume
            if change_weight is not None:
                response_data["change_weight"] = change_weight
            if load_type_id is not None:
                response_data["load_type_id"] = load_type_id
...
async def update_trip(
    ...
    try:
        ...
        loading_place_changed = old_loading_place_id != new_loading_place_id
        unloading_place_changed = old_unloading_place_id != new_unloading_place_id
        ...
        if loading_place_changed or unloading_place_changed:
            ...
            # Синхронизируем историю остатков после изменения ПП/ПР.
            try:
                await place_remaining_service.sync_trip_history_for_cycle(
                    db=session,
                    cycle_id=trip.cycle_id,
                )
            except ValueError as e:
        ...
        # --------------- Пересчет остатков по новому объему/весу ----------------
        # Пересчет допустим только для завершенных рейсов
        new_change_volume = update_data.get("change_volume")
        new_change_weight = update_data.get("change_weight")
        new_load_type_id = update_data.get("load_type_id")
        if new_change_volume is not None and new_change_weight is not None:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Можно изменять только одно поле за раз: change_volume или change_weight",
            )

        if new_change_volume is not None or new_change_weight is not None or new_load_type_id is not None:
            if not (trip.end_time or cycle.cycle_completed_at):
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Пересчет остатков по объему/весу возможен только для завершенных рейсов",
                )
            # Используем сервис остатков по местам для обновления объема/веса и пересчета второй величины
            try:
                await place_remaining_service.update_change_fields_for_trip(
                    db=session,
                    trip=trip,
                    new_change_volume=new_change_volume,
                    new_change_weight=new_change_weight,
                    new_load_type_id=new_load_type_id,
                )
            except ValueError as e:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
