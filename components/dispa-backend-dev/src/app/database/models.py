...
class ShiftTask(Base, TimestampMixin, AuditMixin):
    ...
    staff_id: Mapped[int | None] = mapped_column(index=True)
...
class PlaceRemainingHistory(Base, TimestampMixin):
    ...
    place_id: Mapped[int | None] = mapped_column(
        ...
        nullable=True,
        index=True,
    )  # ID места из graph-service (внешний), может быть не определен
    change_type: Mapped[RemainingChangeTypeEnum] = mapped_column(
        Enum(RemainingChangeTypeEnum, name="remaining_change_type"),
        nullable=False,
    )
    # Объем изменения остатка (м3). Для history без определенного place может быть неизвестен.
    change_volume: Mapped[float | None] = mapped_column(Float, nullable=True)
    # Доп. данные по ВГ (вес груза) и типу груза для пересчета объема.
    load_type_id: Mapped[int | None] = mapped_column(Integer, nullable=True)
    change_weight: Mapped[float | None] = mapped_column(Float, nullable=True)
