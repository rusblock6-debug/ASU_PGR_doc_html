...
class State(StrEnum):
    ...
    @classmethod
    def _missing_(cls, value: object) -> "State | None":
        """Создает динамический custom-state на лету."""
        if isinstance(value, str) and value:
            dynamic_member = str.__new__(cls, value)
            dynamic_member._name_ = "CUSTOM_STATE"
            dynamic_member._value_ = value
...
class StateMachine:
    ...
    async def new_state_action(
        ...
        is_loading_place = place_type in {"load", "reload"}
        is_unloading_place = place_type in {"unload", "reload"}
        ...
        if (
            ...
            and (is_unloading_place or vibro_event == "weight_fall")
            and weight_value is not None
            and weight_value <= settings.end_cycle_weight
        ):
            # Формируем изменение остатка по месту разгрузки
            ...
            and (is_loading_place or vibro_event == "weight_rise")
            ...
            if active_task_data:
                ...
                # Если активного задания нет, очищаем старые значения
                current_state_data.pop("task_id", None)
                current_state_data.pop("shift_id", None)

        elif current_state == State.LOADING and speed_event == "moving" and weight_event == "loaded":
            # Формируем изменение остатка по месту погрузки
            remaining_change = await self._save_place_remaining_history(
                place_id=place_id,
                change_type="loading",
                cycle_id=current_state_data.get("cycle_id"),
            ...
            if remaining_change:
                current_state_data["place_remaining_change"] = remaining_change
                logger.info(
                    "place_remaining_change created for loading",
                    vehicle_id=self.vehicle_id,
                    cycle_id=current_state_data.get("cycle_id"),
                    place_id=place_id,
                    remaining_change=remaining_change,
                ...
                logger.error(
                    "Failed to create place_remaining_change for loading",
                    vehicle_id=self.vehicle_id,
                    cycle_id=current_state_data.get("cycle_id"),
                    place_id=place_id,
                )
    ...
    async def _transition_to_state(
        ...
        current_state_data["previous_state"] = current_state_data.get("state")
        ...
        if db and save_history:
            if (
                current_state_data.get("state") in (State.STOPPED_EMPTY.value, State.STOPPED_LOADED.value)
                and trigger_type == TriggerType.MANUAL
            ):
                history_id = await self._update_last_state_history(
                    new_state=new_state,
    ...
    async def _update_last_state_history(
        self,
        new_state: State,
        state_data: dict[str, Any],
        trigger_type: TriggerType,
        ...
        """Обновить последнюю запись истории вместо создания новой.

        Ищется последняя запись для того же cycle_id (или vehicle_id, если cycle_id отсутствует).
        Обновляются поля: state, state_data, trigger_type, trigger_data, source.
        """
        cycle_id = state_data.get("cycle_id")
        vehicle_id = self.vehicle_id

        # Находим ID последней записи для данного контекста
        query = select(CycleStateHistory.id).where(CycleStateHistory.vehicle_id == vehicle_id)
        if cycle_id:
            query = query.where(CycleStateHistory.cycle_id == cycle_id)
        query = query.order_by(CycleStateHistory.timestamp.desc()).limit(1)

        result = await db.execute(query)
        last_history_id = result.scalar_one_or_none()

        if last_history_id is None:
            # Записи нет – создаём новую
            logger.warning(
                "No history record found for update, creating new one",
                vehicle_id=vehicle_id,
                cycle_id=cycle_id,
            )
            return await self._save_state_history(
                state=new_state,
                state_data=state_data,
                trigger_type=trigger_type,
                trigger_data=trigger_data,
        ...
        source = "manual" if trigger_type == TriggerType.MANUAL else state_data.get("source", "system")
        ...
        stmt = update(CycleStateHistory).where(CycleStateHistory.id == last_history_id).values(**update_values)
        await db.execute(stmt)
        await db.commit()

        logger.info(
            "Updated state history record",
            vehicle_id=vehicle_id,
            history_id=last_history_id,
            old_state=state_data.get("previous_state"),
            new_state=new_state.value,
        )
    ...
    async def _save_place_remaining_history(
        ...
        cargo_type_id = place_info["cargo_type"]
        cargo_type_info = await get_load_type(cargo_type_id)
        ...
        # Рассчитываем объем из ВГ (веса) и плотности:
        # volume (м3) = weight (тонны) / density (т/м3)
        if density == 0 or density < 0:
            logger.error(
                "Density is zero or negative, cannot calculate volume",
                vehicle_id=self.vehicle_id,
                place_id=data_place_id,
                density=density,
                cargo_type=cargo_type_id,
        ...
        # Пока берем табличные данные, после данные с датчиков
        base_weight = load_capacity

        if base_weight < 0:
            logger.error(
                "Base weight is negative for loading",
                vehicle_id=self.vehicle_id,
                change_type=change_type,
        ...
        base_volume = base_weight / density
        if base_volume < 0:
            logger.error(
                "Base volume is negative for loading",
                vehicle_id=self.vehicle_id,
                change_type=change_type,
        ...
        if change_type == "loading":
            change_volume = -base_volume
            change_weight = -base_weight
