...
async def handle_place_remaining_change(
    ...
    Args:
        pr_data: Данные place_remaining_change ({id, place_id, change_type, change_weight, change_volume, load_type_id})
    ...
    try:
        ...
        change_volume = pr_data.get("change_volume")
        load_type_id = pr_data.get("load_type_id")
        change_weight = pr_data.get("change_weight")
        ...
        if change_volume is None:
            missing_fields.append("change_volume")
        ...
        if existing_record:
            ...
            try:
                ...
                if change_type_enum in (
                    RemainingChangeTypeEnum.loading,
                    RemainingChangeTypeEnum.unloading,
                ) and (load_type_id is None or change_weight is None):
                    logger.error(
                        "Trip PlaceRemainingHistory requires load_type_id and change_weight",
                        history_id=history_id,
                        place_id=place_id,
                        change_type=change_type,
                ...
                if not isinstance(change_volume, (int, float)):
                    logger.error(
                        "Invalid change_volume type for PlaceRemainingHistory",
                        history_id=history_id,
                        place_id=place_id,
                        change_volume=change_volume,
                        change_volume_type=type(change_volume).__name__,
                        pr_data=pr_data,
                    )
                    return
                ...
                history = PlaceRemainingHistory(
                    ...
                    change_volume=round(float(change_volume), 1),
                    load_type_id=int(load_type_id) if load_type_id is not None else None,
                    change_weight=round(float(change_weight), 3) if change_weight is not None else None,
...
async def handle_dispatcher_event(topic: str, data: dict[str, Any], db: AsyncSession) -> None:
    r"""Обработчик событий при смене последнего статуса борта \ создании нового статуса борта на КРВ."""
    try:
        # Извлечь vehicle_id из топика dispatcher/{vehicle_id}/changes
        vehicle_id = int(topic.split("/")[1])

        system_name_str = data.get("system_name")

        if system_name_str is None:
            logger.error(
                "System name is missing in dispatcher event",
                vehicle_id=vehicle_id,
            )
            return

        enterprise_statuses = [item.get("system_name") for item in await enterprise_client.get_all_statuses()]
        enterprise_names = {s for s in enterprise_statuses if s}

        if system_name_str in enterprise_names:
            new_state = State(system_name_str)
        else:
            logger.error(
                "Invalid state value received from dispatcher",
                vehicle_id=vehicle_id,
                invalid_state=system_name_str,
            )
            return

        new_data = {
            "new_state": new_state,
            "reason": "dispatcher_action",
            "comment": "Действие диспетчера",
        }
        state_machine = get_state_machine(vehicle_id)

        logger.warning(
            "Manual dispatcher state transition requested",
            vehicle_id=vehicle_id,
            new_state=new_data.get("new_state"),
            reason=new_data.get("reason"),
        )
        await state_machine.manual_transition(
            new_state=new_state,
            reason=str(new_data.get("reason")),
            comment=str(new_data.get("comment")),
            db=db,
        )

        logger.info(
            "[BORT] State changed by dispatcher event",
            topic=topic,
        ...
        logger.error(
            "Error handling dispatcher mqtt event",
            topic=topic,
            error=str(e),
            exc_info=True,
        )
    ...
    try:
        ...
        try:
            ...
            if settings.service_mode == "bort":
                ...
                elif "wifi/fake_events" in topic:
                    await _publish_tag_to_redis(topic, sensor_data)
