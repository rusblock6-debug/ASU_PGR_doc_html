...
async def _run_full_shift_state_calculation() -> None:
    ...
    current_shift_data: dict[int, dict[str, object]] = {}
    ...
    while True:
        try:
            ...
            async with AsyncSessionLocal() as session:
                shift_service = ShiftTaskService(session)
                changed_regimes = await shift_service.is_shift_changed(current_shift_data)
                if changed_regimes:
                    if settings.shift_auto_switch:
                        current_shift_data = await shift_service.copy_from_previous_shift(
                            current_shift_data,
                            changed_regimes=changed_regimes,
                        )
                    await shift_service.complete_previous_shift_route_tasks(
                        current_shift_data,
                        changed_regimes=changed_regimes,
                    )
                    current_shift_data.update(changed_regimes)
            logger.info(
