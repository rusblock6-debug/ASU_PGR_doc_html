...
def _normative_weight_tons(vehicle_payload: dict[str, Any] | None) -> float | None:
    """Грузоподъёмность модели техники (т), из enterprise-service."""
    if not vehicle_payload:
        return None
    model = vehicle_payload.get("model")
    if not isinstance(model, dict):
        return None
    raw = model.get("load_capacity_tons")
    if raw is None:
        return None
    try:
        return float(raw)
...
async def get_vehicle_tooltip(
    ...
    vehicle_payload, telemetry_rows = await asyncio.gather(
        enterprise_client.get_vehicle_by_id(vehicle_id),
        asyncio.gather(
            _read_last_stream_row(_TELEMETRY_SPEED_PREFIX, vehicle_id),
            _read_last_stream_row(_TELEMETRY_GPS_PREFIX, vehicle_id),
        ),
    )
    (s_val, s_ts), (_g_val, g_ts) = telemetry_rows
    weight = _normative_weight_tons(vehicle_payload)
    speed = s_val

    ts_candidates = [t for t in (g_ts, s_ts) if t is not None]
    if ts_candidates:
