...
async def get_shift_load_type_volumes(
    ...
    *,
    ...
    Вид груза: cargo_type места погрузки (loading_place_id), как при расчёте change_volume.
    ...
    query = (
        select(
            ...
            PlaceRemainingHistory.load_type_id,
        ...
        .group_by(Trip.loading_place_id, PlaceRemainingHistory.load_type_id, PlaceRemainingHistory.place_id)
    )

    result = await db.execute(query)
    grouped_rows: list[tuple[int, int | None, int, float]] = []
    for loading_pid, load_type_id, unloading_pid, total in result.all():
        if total is None:
            continue
        grouped_rows.append(
            (
                int(loading_pid),
                int(load_type_id) if load_type_id is not None else None,
                int(unloading_pid),
                float(total),
            ),
        )
    ...
    unique_unloading = {r[2] for r in grouped_rows}
    ...
    for loading_pid, row_load_type_id, unloading_pid, vol in grouped_rows:
        lt_id = row_load_type_id if row_load_type_id is not None else loading_to_cargo.get(loading_pid)
        if lt_id is None:
            continue
