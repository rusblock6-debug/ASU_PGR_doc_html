...
import { type DeleteToastMessages, deleteWithToast } from '../lib/table-actions';
import type { ColumnDef } from '../types';

export type DrawerMode = 'closed' | 'add' | 'edit';

/**
 * Представляет значение контекста таблицы.
 */
...
/**
 * Представляет свойства компонента {@link TableProvider}.
 */
interface TableProviderProps<TData> {
  ...
  /** Кастомные сообщения для уведомлений при удалении. */
  readonly deleteToastMessages?: DeleteToastMessages;
}

const TableContext = createContext<TableContextValue<unknown> | null>(null);

/**
 * Представляет провайдер контекста таблицы.
 */
export function TableProvider<TData>({
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((row) =>
      columns.some((col) => {
        ...
        const formatted = formatValue(value, col.meta?.dataType);
        return hasValueNotEmpty(formatted) && formatted.toLowerCase().includes(lowerQuery);
      }),
    );
  ...
  const wrappedOnDelete = useCallback(
    async (ids: (string | number)[]) => {
      if (!onDelete) return;
      await deleteWithToast(onDelete(ids), ids.length, deleteToastMessages);
      onSelectRow?.([]);
    },
    [onDelete, onSelectRow, deleteToastMessages],
  );
  ...
  return <TableContext.Provider value={value as TableContextValue<unknown>}>{children}</TableContext.Provider>;
}

/**
 * Возвращает значение контекста таблицы.
 */
