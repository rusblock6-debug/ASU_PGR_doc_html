...
import { MAC_ADDRESS_PLACEHOLDER, MAC_ADDRESS_REGEX } from '@/shared/lib/format-mac-address';
import { hasValue, hasValueNotEmpty } from '@/shared/lib/has-value';
import { EMPTY_COORDINATES } from '@/shared/models/LocationModel';
...
/**
 * Валидация MAC-адреса в формате {@link MAC_ADDRESS_PLACEHOLDER}.
 */
export const MAC_ADDRESS_VALIDATION = STRING_VALIDATION.regex(MAC_ADDRESS_REGEX, {
  message: `Неверный формат MAC-адреса (${MAC_ADDRESS_PLACEHOLDER})`,
});
...
export function withRequired<T extends z.ZodType>(schema: T) {
  ...
  return (required: boolean): z.ZodType => (required ? schema : z.preprocess(toUndefinedIfEmpty, schema.optional()));
}
