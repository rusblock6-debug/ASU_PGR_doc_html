 ...
 */
export function toTimezone(date: Date | string, timezone: string | undefined | null): Date {
  if (!timezone) {
 ...
 */
export function getNowInTimezone(timezone: string | undefined | null): Date {
  if (!timezone) {
    return new Date();
  }
 ...
 */
export function toUTC(date: Date | string, timezone: string | undefined | null): Date {
  if (!timezone) {
    return typeof date === 'string' ? new Date(date) : date;
  }
 ...
 */
export function getNowInTimezoneAsUTC(timezone: string | undefined | null): string {
 ...
 */
export function utcToDatetimeLocal(utcString: string, timezone: string | undefined | null): string {
 ...
 */
export function datetimeLocalToUTC(datetimeLocal: string, timezone: string | undefined | null): string {
 ...
 * @returns UTC ISO строка для отправки на сервер
 */
export function dateTimeInputsToUTC(dateInput: string, timeInput: string, timezone: string | undefined | null): string {
 ...
 */
export function createLocalDateWithTimezoneValues(date: Date | string, timezone: string | undefined | null): Date {
  const dateObj = typeof date === 'string' ? new Date(date) : date;
 ...
 * @returns UTC Date
 */
export function localDateWithTimezoneValuesToUTC(localDate: Date, timezone: string | undefined | null): Date {
  if (!timezone) {
    return localDate;
  }
 ...
 */
export function normalizeISODateTime(value: Date | string): string {
 ...
 */
export function normalizeISODateTimeWithSeconds(value: Date | string): string {
  const date = typeof value === 'string' ? new Date(value) : value;
  return formatInTimeZone(date, 'UTC', "yyyy-MM-dd'T'HH:mm:ss.000'Z'");
}

/**
 * Нормализует ISO дату, обрезая миллисекунды.
 * «2025-12-02T20:53:42.012208Z» → «2025-12-02T20:53:42Z»
 */
export function normalizeISODateTimeWithoutMilliseconds(value: Date | string): string {
  const date = typeof value === 'string' ? new Date(value) : value;
