...
import type { EdgeGraph, HorizonGraphResponse, UpdateHorizonGraphRequest } from '@/shared/api/endpoints/horizons';
...
const EDGE_TYPE_MAP = {
  horizontal: GraphElementType.ROAD,
  vertical: GraphElementType.LADDER,
} as const satisfies Record<EdgeGraph['edge_type'], string>;
 ...
 * 3. Обход рёбер: серверные ссылки `from_node_id`/`to_node_id` конвертируются через маппинг в клиентские `fromId`/`toId`,
 *    каждому ребру присваивается клиентский `tempId` через nanoid, серверный `id` сохраняется.
 *    Рёбра, ссылающиеся на отсутствующие узлы и петли отбрасываются.
 *
 * @example
 * ```
...
export function serverToEditor(server: HorizonGraphResponse) {
  ...
  const nodes = server.nodes.map((node) => {
    ...
    return { id: node.id, tempId, x, z, horizonId: node.horizon_id, nodeType: node.node_type };
  });

  const edges = server.edges.reduce<GraphData['edges'][number][]>((result, edge) => {
    const fromId = idToTempId.get(edge.from_node_id);
    const toId = idToTempId.get(edge.to_node_id);

    if (fromId && toId && fromId !== toId) {
      result.push({ id: edge.id, tempId: nanoid(), fromId, toId, edgeType: EDGE_TYPE_MAP[edge.edge_type] });
    }
