...
import { useMapVehicleRealtimeState } from '../../../lib/hooks/useMapVehicleRealtimeState';
import { selectMapFocusTarget, selectSelectedHorizonId } from '../../../model/selectors';
...
export function CameraController() {
  ...
  const { data: vehicleMapState } = useMapVehicleRealtimeState();
  ...
  useFrame((state, delta) => {
    ...
    if (focusTarget.entity === 'vehicle') {
      const vehicle = vehicleMapState[focusTarget.id];
      if (!vehicle || vehicle.horizon_id !== horizonId || !hasValue(vehicle.place_id)) return;
    }

    const pos = resolveTargetPosition(focusTarget, vehicleMapState, placesData?.items);
    if (!pos) return;
