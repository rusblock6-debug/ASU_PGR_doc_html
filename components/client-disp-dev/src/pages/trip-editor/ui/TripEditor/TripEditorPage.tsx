...
import {
  ...
  getDateRangeFromSession,
  saveDateRangeToSession,
  ShiftFilter,
} from '@/features/shift-filter';
...
const deleteToastMessages = {
  loading: (count: number) => {
    return `Удаление ${count}\u00A0${ruPlural(count, 'рейса', 'рейсов', 'рейсов')}`;
  },
  success: (count: number) => {
    return `${count}\u00A0${ruPlural(count, 'рейс', 'рейса', 'рейсов')} ${ruPlural(count, 'удалён', 'удалены', 'удалено')}`;
  },
};
...
export function TripEditorPage() {
  ...
  const handleAdd = async (newTrip: Partial<Trip>) => {
    ...
    const request = createTrip({
      ...
      cycle_started_at: cycleStartedAt,
      cycle_completed_at: cycleCompletedAt,
      change_volume: hasValue(changeVolume) ? changeVolume : null,
      change_weight: hasValue(changeWeight) ? changeWeight : null,
    }).unwrap();
  ...
  const handleEdit = async (id: string | number, updatedData: Partial<Trip>) => {
    await updateTrip({
      ...
      body: {
        ...
        change_volume: hasValue(updatedData.change_volume) ? updatedData.change_volume : null,
        change_weight: hasValue(updatedData.change_weight) ? updatedData.change_weight : null,
      },
    }).unwrap();
  };
  ...
  return (
    <Page variant="table">
      <TableProvider
        ...
        deleteToastMessages={deleteToastMessages}
