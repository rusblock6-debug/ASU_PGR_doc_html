...
Приложение использует переменные окружения для указания API. По умолчанию без создания `.env` будет использовать адрес на дев-сервере.
Чтобы переопределить — выполните в корне проекта (это создаст файл `.env`):

```bash
cp .env.example .env
```

Файл `.env.example` содержит дефолтные адреса API для докер контейнера при разворачивании всей системы локально через [управляющий репозиторий](https://git.dmi-msk.ru/retek.pgr/dispatching/dispatching-repo).
